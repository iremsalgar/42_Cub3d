...
static int	is_color(char *identifier)
{
		...
		)
{
	...
	if (ft_strcmp(identifier, "NO") == 0)
		scene_description->texture[NORTH] = ft_strdup(path);
	else if (ft_strcmp(identifier, "SO") == 0)
		scene_description->texture[SOUTH] = ft_strdup(path);
	else if (ft_strcmp(identifier, "WE") == 0)
		scene_description->texture[WEST] = ft_strdup(path);
	else if (ft_strcmp(identifier, "EA") == 0)
		scene_description->texture[EAST] = ft_strdup(path);
	else
		return (print_error_return(INVALID_IDENTIFIER_ERROR, 1));
		...
		)
{
	...
	i = 0;
	while (*scene_content && (*scene_content)[i] != (void *)0 \
